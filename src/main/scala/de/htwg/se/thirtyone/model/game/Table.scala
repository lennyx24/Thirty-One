  ...
  def printTable(players: List[Player]): String = {
    ...
    grid.zipWithIndex.foldLeft("") { case (output, (row, rowIndex)) =>
      val playerHeader = rowIndex match {
        ...
        case 2 =>
          val p4 = if (players.length > 3) s"${players(3).name}: ${players(3).playersHealth} Leben, ${players(3).points} Punkte" else ""
          val p3 = if (players.length > 2) s"${players(2).name}: ${players(2).playersHealth} Leben, ${players(2).points} Punkte" else ""
