...
class SwapStateSpec extends AnyWordSpec with Matchers {
  "SwapState" should {
    ...
    def makeController(state: de.htwg.se.thirtyone.controller.state.SwapState = new de.htwg.se.thirtyone.controller.state.SwapState): GameController = {
    ...
    "handle give then take swap" in {
      ...
      controller.state shouldBe de.htwg.se.thirtyone.controller.state.PlayingState
    }

    "handle invalid input" in {
      events.clear()
      val controller = makeController(new de.htwg.se.thirtyone.controller.state.SwapState)

      controller.state.asInstanceOf[de.htwg.se.thirtyone.controller.state.SwapState].execute("x", controller)
